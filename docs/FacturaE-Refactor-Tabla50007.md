...
Así, otros objetos que antes llamaban a la tabla pueden migrarse gradualmente.
...
La tabla 50007 se mantiene como cabecera de factura recibida, pero los campos se han renombrado a un estilo más legible AL:

| Antes NAV 2016 | Ahora AL |
|---|---|
| `ID_PLATAFORMA` | `ID Plataforma` |
| `NUM` | `Numero` |
| `SERIE` | `Serie` |
| `FECHA_ENTRADA` | `Fecha Entrada` |
| `FECHA_DEVENGO` | `Fecha Devengo` |
| `EMISOR_CIF` | `Emisor CIF` |
| `EMISOR_NOMBRE` | `Emisor Nombre` |
| `CCC_PAGO` | `IBAN Pago` |
| `TOTAL_BASES` | `Total Bases` |
| `TOTAL_TASAS` | `Total Tasas` |
| `TOTAL_PAGAR` | `Total Pagar` |

Además, se añade `DataClassification` a los campos para cumplir requisitos de Business Central SaaS.
...
2. **Trazabilidad**: cada importación puede quedar registrada en log.
3. **Flexibilidad**: se puede importar manualmente o desde API.
4. **Menos riesgo**: la tabla deja de concentrar toda la lógica crítica.
5. **Mantenible**: cada responsabilidad está en un objeto específico.
6. **Extensible**: se pueden añadir integraciones mediante eventos o endpoint HTTP.
7. **Auditable**: se guarda el XML original en Business Central.
8. **Operativo para usuarios**: hay páginas para importar, configurar y revisar logs.
